---
name: pdf
description: 当用户想要对 PDF 文件做任何操作时使用此技能。包括：读取或提取 PDF 中的文本/表格、合并多个 PDF 为一个、拆分 PDF、旋转页面、添加水印、创建新 PDF、填写 PDF 表单、加密/解密 PDF、提取图片，以及对扫描 PDF 进行 OCR 使其可搜索。如果用户提到 .pdf 文件或要求生成 PDF，使用此技能。
...
本指南涵盖使用 Python 库和命令行工具的基本 PDF 处理操作。高级功能、JavaScript 库和详细示例请参见 REFERENCE.md。如果你需要填写 PDF 表单，请阅读 FORMS.md 并遵循其说明。
...
**重要**：绝不要在 ReportLab PDF 中使用 Unicode 下标/上标字符（₀₁₂₃₄₅₆₇₈₉, ⁰¹²³⁴⁵⁶⁷⁸⁹）。内置字体不包含这些字形，会导致它们渲染为纯黑方块。
...
# 这将所有图片提取为 output_prefix-000.jpg, output_prefix-001.jpg 等。
```
